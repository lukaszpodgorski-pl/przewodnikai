...
![Czysta mapa węzłów typów systemów AI połączonych świetlistymi trasami](../../../assets/jak-dziala-ai/mapa-ai/mapa-ai.jpg)
...
Zacznij od tej sekcji, nawet jeśli resztę mapy tylko przewiniesz wzrokiem. Te trzy elementy to podstawa: model, który generuje odpowiedzi, i dwa sposoby na dostarczenie mu wiedzy - z Twoich dokumentów i z poprzednich rozmów.

<CardGrid>
	<Card title="LLM (Large Language Models)" icon="open-book">
		Model wytrenowany na ogromnej ilości tekstu, który przewiduje kolejne tokeny i dzięki temu potrafi pisać, streszczać i rozumować w języku naturalnym. To silnik, z którego korzystasz w ChatGPT czy Claude. Cały rozdział: [Modele językowe LLM](/jak-dziala-ai/modele-jezykowe/).
	...
	<Card title="RAG (Retrieval Augmented Generation)" icon="magnifier">
		Sam model zna tylko to, co było w danych treningowych. RAG dokłada mu wyszukiwarkę: zanim odpowie, znajduje właściwe fragmenty Twoich dokumentów i opiera odpowiedź na nich, zamiast zgadywać. Cały rozdział: [RAG - AI na Twoich dokumentach](/jak-dziala-ai/rag/).

		**Zastosowania:** asystent po firmowej bazie wiedzy, analiza dokumentów prawnych, przegląd literatury naukowej, obsługa klienta.
	</Card>
	<Card title="Pamięć długoterminowa" icon="document">
		Zwykła rozmowa z modelem kończy się razem z sesją. Pamięć zapisuje, co już ustaliliście, i wraca do tego w kolejnych rozmowach - zamiast pytać Cię o to samo od nowa.
...
<CardGrid>
	<Card title="Agenci AI (agentic AI)" icon="rocket">
		Dostają cel zamiast pojedynczego polecenia. Sami planują kroki, wykonują je, sprawdzają wynik i poprawiają się po drodze - bez Twojej akceptacji na każdym etapie. To samo kryje się pod hasłami "autonomiczni agenci", "agenci rozumujący i planujący" oraz "autonomiczne podejmowanie decyzji". Cały rozdział: [Agenci AI](/jak-dziala-ai/agenci-ai/).

		**Zastosowania:** asystent badawczy zbierający i porządkujący materiały, automatyzacja wieloetapowego workflow, przeglądanie stron i wypełnianie formularzy za Ciebie, samodzielne wprowadzanie zmian w plikach projektu.
	</Card>
...
<CardGrid>
	<Card title="MCP (Model Context Protocol)" icon="puzzle">
		Otwarty standard opisujący, jak model ma sięgać do zewnętrznych narzędzi, danych i usług. Zamiast pisać osobną integrację pod każdy model, podłączasz narzędzie raz - i rozumie je każdy klient obsługujący MCP. Praktyczny przewodnik: [Serwery MCP](/narzedzia/serwery-mcp/).
	...
	<Card title="Wywoływanie narzędzi (tool calling / tool use)" icon="setting">
		Mechanizm, w którym model zamiast odpowiadać tekstem zgłasza: "wywołaj tę funkcję z tymi argumentami". _Tool calling_ i _tool use_ to dwie nazwy tego samego mechanizmu: OpenAI mówi zwykle o wywoływaniu funkcji (_function calling_), Anthropic o użyciu narzędzi (_tool use_). Sytuację, w której agent sam wybiera i łączy kolejne narzędzia krok po kroku, opisuje osobne pojęcie - pętla decyzyjna agenta (_agentic loop_), którą poznałeś w rozdziale o [agentach AI](/jak-dziala-ai/agenci-ai/).
...
**Następny krok:** [Prompt Engineering](/prompt-engineering/) - czas przejść do praktyki! Nauczysz się skutecznie komunikować z AI i pisać prompty, które dają najlepsze rezultaty.
:::
